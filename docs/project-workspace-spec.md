...
- 校验范围包括 `.moyuan/workspace.json`、`project.yaml`、`repository.yaml`、`policies/access.yaml` 的存在性、YAML 解析、核心必填字段、条件必填、必须为空和关键字段漂移；`.moyuan/models/providers.yaml` 和 `.moyuan/models/routing.yaml` 存在时也会校验 provider/routing schema 和 secret 引用规则。
