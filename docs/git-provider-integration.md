...
- CLI：`moyuan git provider plan <issue-id>`、`list`、`show <plan-id>`、`sync <plan-id>`、`preview <plan-id>`、`create <plan-id> [--approved] [--approval-id <approval-id>]`。
...
- Phase 5 已支持 GitHub/Gitee PR/MR create adapter：必须 `--approved`、`--approval-id` 指向已批准且 target/action 匹配的 approval record、authz `git:write`、`secret` resolver 通过，并设置 `MOYUAN_ALLOW_GIT_PROVIDER_WRITE=1` 才会调用远程 API；否则保持 approval-required 或 preview-only。
- Phase 8 已支持 GitHub/Gitee release provider create release adapter：必须通过 `release.provider.publish` approval、`MOYUAN_ALLOW_RELEASE_PROVIDER_WRITE=1`、`secret` resolver 和 replay guard；当前只真实调用 create release API，branch/tag/workflow action 仍显式跳过。
