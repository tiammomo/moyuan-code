...
当前已完成 Phase 1 本地 CLI MVP、Beta 控制面能力、Phase 2 到 Phase 27 的主要闭环；Phase 27 已通过 readiness 收口。
...
- 当前实现重点：Phase 27 已完成 readiness 收口，`server_resource_registry_adapter` 已支持本地 apply receipt，`ssh_deployment_adapter` 已支持执行前 sandbox 和 rollback binding，失败/人工 adapter execution 会生成 recovery record 并可绑定 control queue 人工复核；SSH/GitHub/Gitee/cloud 仍不直接执行外部真实写入。
...
./bin/moyuan operations write-adapter-recoveries list --root /path/to/repo
./bin/moyuan control-loop queue add --adapter-recovery-id <write-adapter-recovery-id> --maintenance-window always --root /path/to/repo
