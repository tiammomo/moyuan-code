...
- 完整配置：[完整配置方案](./configuration-guide.md)
- 配置字段规则：[配置 Schema 规则](./configuration-schema-spec.md)
- 仓库、Git、项目理解：[仓库接入、Git 与项目理解](./repository-onboarding-git-management.md)
- Agent、team、skills：[Agent、Skills 与编排](./agent-skills-memory.md)
...
| 全套配置示例 | 初始化项目所需完整 YAML 配置 | [完整配置方案](./configuration-guide.md) |
...
| `visuals/` | 架构图 spec、prompt、图片资产、讲解文档和索引 | [完整配置方案](./configuration-guide.md) |
...
| `resources/` | 服务器资产清单、巡检、续费、成本和变更事件 | [完整配置方案](./configuration-guide.md) |
...
| `policies/logging.yaml` | 核心日志、审计日志、脱敏、保留和导出策略 | [完整配置方案](./configuration-guide.md) |
| `policies/server-resources.yaml` | 测试开发机、生产机、云资产、到期时间、资源组、连接、权限和健康检查 | [完整配置方案](./configuration-guide.md) |
| `policies/release.yaml` | 版本分支、tag、PR/MR、投产和回滚策略 | [Issues 编排与并发调度](./issue-orchestration.md) |
| `policies/environments.yaml` | 服务器、环境、部署方式、健康检查、监控配置引用 | [Issues 编排与并发调度](./issue-orchestration.md) |
...
- 仓库和项目阅读理解只在 [仓库接入、Git 与项目理解](./repository-onboarding-git-management.md) 展开。
