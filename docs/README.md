...
当前阶段：Phase 18 已启动，operations timeline、maintenance policy pack、post-deployment verification、服务器生命周期控制和 Console 运维 dashboard 已完成，后续聚焦 Phase 18 readiness 收口。Phase 17 release admission policy pack、bounded rehearsal scheduler、risk review queue 和 Console drill-down 已完成 readiness。
...
Phase 1 本地 CLI MVP、Beta 控制面能力、Phase 2 到 Phase 17 已完成主要闭环。Phase 18 已进入执行。当前验收和状态入口：

- [Phase 18 实现 Issue Graph](./phases/phase18-issue-graph.md)：生产运维闭环与策略化维护控制面的依赖图。
- [Phase 18 实施记录](./phases/phase18-next-development-plan.md)：Phase 18 当前任务、验收标准和执行入口。
