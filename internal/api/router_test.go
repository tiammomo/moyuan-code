...
func TestGinRouterServesProjectStateEndpoints(t *testing.T) {
	...
	assertPostContains(t, router, "/v1/projects/managed/release-candidates/"+releaseCandidates[0].ID+"/provider-publish", `{}`, http.StatusAccepted, `"release_provider_execution"`, `"RELEASE_CANDIDATE_PROVIDER_PUBLISH_BLOCKED"`, "release_candidate_not_ready:")
	...
	assertGETContains(t, router, "/v1/projects/managed/operations/timeline?limit=20&type=release_admission", http.StatusOK, `"operations_timeline"`, `"release_admission"`, admissions[0].ID, `"evidence_refs"`)
	...
	assertGETContains(t, router, "/v1/projects/managed/operations/write-proofs?limit=20&operation_type=deployment_execution", http.StatusOK, `"write_proofs"`, `"deployment_execution"`, evidenceRecords[0].ParentID, `"WRITE_PROOF_WRITE_DISABLED"`)
	assertGETContains(t, router, "/v1/projects/managed/operations/write-admissions?limit=20&operation_type=deployment_execution", http.StatusOK, `"write_admissions"`, `"deployment_execution"`, evidenceRecords[0].ParentID, `"WRITE_ADMISSION_WRITE_DISABLED"`)
	assertGETContains(t, router, "/v1/projects/managed/operations/provider-proof-requirements?limit=20&provider=local&operation_type=deployment_execution", http.StatusOK, `"provider_proof_requirements"`, `"provider":"local"`, `"PROVIDER_PROOF_REQUIREMENT_SUPPORTED"`)
	assertPostContains(t, router, "/v1/projects/managed/operations/remote-execution-rehearsals", `{"execution_id":"`+evidenceRecords[0].ParentID+`","limit":5}`, http.StatusAccepted, `"remote_execution_rehearsals"`, `"remote_write_never_executed"`)
