...
| --- | --- | --- | --- | --- | --- | --- |
| `phase13-001` | `release-candidate-plan-from-batch` | planned | release candidate 事实源、从 release batch plan 读取 source/release branch/version、provider/remote/deploy target 摘要 | Phase 12 readiness | `release_owner` + `backend_owner` | suggested release batch 可生成 release candidate plan，不执行 Git 或远程写入 |
