...
- Phase 5 已支持 GitHub/Gitee create adapter：必须 `--approved`、`--approval-id` 指向已批准且 target/action 匹配的 approval record、authz `git:write`、`secret` resolver 通过，并设置 `MOYUAN_ALLOW_GIT_PROVIDER_WRITE=1` 才会调用远程 API；否则保持 approval-required 或 preview-only。
