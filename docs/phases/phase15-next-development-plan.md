...
| P1 | `phase15-004` | `console-deployment-ops-surface` | completed | Console 运维操作面 | 展示 approval、rollback、monitor 事实源 |
| P2 | `phase15-005` | `phase15-readiness` | completed | Phase 15 收口 | 全量门禁和风险清单完成 |
...
## 6. 执行规划：`phase15-004 console-deployment-ops-surface`

实现状态：completed。

范围：

- Console snapshot 接入 rollback execution list 和 monitor summary list。
- Deployment execution 列表展示 `approval_id` 和 `approval_consumed`。
- Deployment 操作区新增 rollback preview 和 monitor summary 受控入口。
- 维护摘要区展示最近 monitor summary、rollback execution 和 post-deployment history。

非目标：

- 不在 Console 执行真实 rollback。
- 不在前端消费 approval。
- 不在前端计算生产健康结论。

验收：

- Console 只调用后端 `rollback preview` 和 `monitor summary` API。
- rollback、monitor 和 approval 状态均以后端返回的 fact source 为准。
- Demo snapshot 和 live snapshot 具备同构字段，前端 typecheck/build 通过。

落地结果：

- `ConsoleSnapshot` 增加 `rollback_executions` 和 `monitor_summaries`。
- Console Deployment Executions 面板能触发 `Rollback Preview` 和 `Monitor Summary`。
- monitor summary 显示窗口、失败数和 rollback 数，rollback execution 显示 mode、step count 和 approval 信息。

## 7. 执行规划：`phase15-005 phase15-readiness`

实现状态：completed。

范围：

- 新增 Phase 15 release readiness。
- 统一 Phase 15 issue graph、实施记录、docs 入口和路线图状态。
- 明确 production real execution、rollback execution、monitor summary 和 Console 的保留边界。
- 给出 Phase 16 建议入口。

落地结果：

- Phase 15 状态收敛为 `ready`。
- 下一阶段建议聚焦部署演练、运行风险闭环与发布准入增强。

## 8. 后续执行占位

Phase 15 完成后，应打开 Phase 16 issue graph，再进入下一批代码实现。
