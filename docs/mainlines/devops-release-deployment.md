...
- CLI：`moyuan deploy plan <release-id> --environment <env> [--resource <host-id>]`、`moyuan deploy execute <deployment-id> [--mode dry_run|ssh_preview|ssh_execute|local_shell] [--approved] [--approval-id <approval-id>]`、`moyuan deploy rollback <execution-id> [--mode preview|local_shell] [--approved] [--approval-id <approval-id>]`、`moyuan deploy monitor summarize [--environment <env>] [--limit <n>]`、`moyuan deploy verify create --execution-id <id> [--environment <env>] [--monitor-limit <n>]`、`moyuan deploy verify list/show`、`moyuan deploy show <deployment-id>`。
- API：`POST /v1/projects/:project_id/releases/suggest`、`GET /v1/projects/:project_id/releases/:release_id`、`POST /v1/projects/:project_id/releases/:release_id/provider-preview`、`POST /v1/projects/:project_id/releases/:release_id/provider-publish`、`GET /v1/projects/:project_id/release-provider-executions`、`GET /v1/projects/:project_id/release-provider-executions/:execution_id`、`POST /v1/projects/:project_id/deployments/plan`、`GET /v1/projects/:project_id/deployments/:deployment_id`、`POST /v1/projects/:project_id/deployments/:deployment_id/execute`、`POST /v1/projects/:project_id/deployment-executions/:execution_id/rollback`、`GET /v1/projects/:project_id/deployment-rollback-executions`、`GET /v1/projects/:project_id/deployment-monitor-history`、`POST /v1/projects/:project_id/deployment-monitor-summary`、`GET /v1/projects/:project_id/deployment-monitor-summaries`、`GET /v1/projects/:project_id/deployment-executions/:execution_id/post-deployment-history`、`POST /v1/projects/:project_id/post-deployment-verifications`、`GET /v1/projects/:project_id/post-deployment-verifications`、`GET /v1/projects/:project_id/post-deployment-verifications/:verification_id`、`GET /v1/projects/:project_id/evidence`、`GET /v1/projects/:project_id/operations/:operation_type/:operation_id`。
...
- 当前生成 release suggestion、release branch plan、tag suggestion、release notes draft、provider release/tag/workflow action preview、deploy/smoke/monitor/rollback plan；smoke/monitor plan 会引用检查模板并携带 severity、window 和 failure classes。受控 `local_shell` 必须携带并消费匹配 `approval_id`，执行后自动记录 smoke/monitor 结果；`ssh_preview` 可生成远程目标执行预览，`ssh_execute` 默认不开启真实 SSH 连接或生产部署。
...
- SSH execute 已具备受控真实执行 Beta：默认返回 blocked；设置 `MOYUAN_ALLOW_SSH_EXECUTE=1` 后仍必须携带并消费匹配 `approval_id`，再校验 server resource、`auth_ref`、命令 allowlist 和超时，通过本机 `ssh` 二进制执行。stdout/stderr 写入 execution 前脱敏，成功后自动进入 smoke/monitor，失败后生成 rollback suggestion；production real execution 仍继续阻断。
...
- Phase 12 已增加 release batch readiness：基于 completed integration apply 和 ready integration items 数量生成 release batch plan，输出到 `.moyuan/lifecycle/releases/batches/` 和 `.moyuan/lifecycle/releases/release-batches.jsonl`；该阶段只给出发版批次建议，不创建 release branch、不 tag、不 push。
...
- Rollback execution 已具备受控 preview/local_shell Beta：默认只从 runbook 生成 preview；真实 local shell 回退必须通过 approval、`MOYUAN_ALLOW_ROLLBACK_EXECUTE=1`、命令 allowlist 和 approval consumption，并写入独立 evidence。
- Monitor summary 已具备窗口聚合 Beta：从 post-deployment history 汇总最近窗口的 failed、blocked、manual、rollback 和 failure class，输出 deployment 或 production 风险结论并写入独立 evidence。
- Post-deployment verification 已具备线上验证闭环：按 deployment execution 聚合 smoke report、post-deployment history、monitor summary、rollback suggestion 和 evidence；失败或关注态只输出 `risk_handoff_recommended` 和风险来源，不自动执行生产修复。
...
.moyuan/lifecycle/deployments/post-deployment-verifications/
...
- 冒烟失败或监控异常会进入回滚或人工介入。
- 发布后 verification 失败只能进入风险复核或人工介入，不能自动生产修复。
