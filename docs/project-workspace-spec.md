...
- 校验范围包括 `.moyuan/workspace.json`、`project.yaml`、`repository.yaml`、`policies/access.yaml` 的存在性、YAML 解析、核心必填字段、条件必填、必须为空和关键字段漂移；`.moyuan/models/providers.yaml`、`.moyuan/models/routing.yaml`、`.moyuan/visuals/architecture-visuals.yaml` 和 `.moyuan/runtimes/agent-runtimes.yaml` 存在时也会校验对应 schema 和 secret 引用规则。
