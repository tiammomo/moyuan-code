...
- 当前生成 release suggestion、release branch plan、tag suggestion、release notes draft、deploy/smoke/monitor/rollback plan，并在受控 `local_shell` 执行后自动记录 smoke/monitor 结果；`ssh_preview` 可生成远程目标执行预览，`ssh_execute` 仍默认阻断，不真实创建 branch、tag、远程 release、SSH 或生产部署。
