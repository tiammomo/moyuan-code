...
| P1 | `phase18-005` | `console-operations-dashboard` | completed | Console 运维 dashboard | 展示 timeline 和资源风险 |
| P2 | `phase18-006` | `phase18-readiness` | completed | Phase 18 收口 | 全量门禁和生产边界完成 |
...
- Console 拉取后端 `operations/timeline`，Operation History 优先展示统一 timeline；未连接后端时回退本地聚合。
- Console 拉取 post-deployment verification 和 resource deployment refs。
- Deployments 面板新增 `Verify` 受控动作，调用后端 post-deployment verification API。
- Deployments 面板展示 verification decision、monitor decision 和 risk handoff recommendation。
- Server Resources 面板展示 resource `last_deployment` 和最近 resource deployment refs。
- 新增 `Operations` 导航入口，复用后端事实源，不在前端重算准入、维护策略或资源 readiness。

## 8. 执行规划：`phase18-006 phase18-readiness`

实现状态：completed。

范围：

- 汇总 Phase 18 完成能力、全量门禁、保留边界和 Phase 19 入口。
- 回查 docs 当前状态，确保没有“规划中”表述误导后续开发。
- 明确生产真实写入仍默认关闭，Console 仍只触发受控 API。

验收：

- 新增 Phase 18 readiness 文档。
- README、docs README、phase index 和主线文档状态一致。
- 全量门禁通过。

完成记录：

- 新增 [Phase 18 Release Readiness](./phase18-release-readiness.md)，汇总完成范围、门禁结论、保留边界和 Phase 19 入口。
- README、docs README、phase index、issue graph、执行记录和 lifecycle roadmap 已统一到 Phase 18 readiness 完成状态。
- 生产真实写入默认关闭、Console 只消费后端事实源和受控 API 的边界已回写到收口文档。
