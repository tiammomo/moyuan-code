...
| P1 | `phase17-003` | `risk-review-queue` | completed | 风险复核队列 | handoff 可 approved/rejected/defer |
| P1 | `phase17-004` | `console-policy-risk-drilldown` | completed | Console drill-down | 展示 policy、scheduler 和 risk review |
| P2 | `phase17-005` | `phase17-readiness` | completed | Phase 17 收口 | 全量门禁和自动化边界完成 |
...
- 新增 deployment risk review 记录，复核动作和 handoff 源事实分离。
- 新增 pending/reviewed/all review queue，可查询 handoff、signal、bug candidate、repair plan 和 evidence。
- 支持 `approved`、`rejected`、`deferred`；deferred 保持可复核，approved/rejected 关闭复核。
- Review 写入 `.moyuan/repair/deployment-risk-reviews/`、JSONL、run log 和 evidence，并回写 handoff 最新复核状态。
- CLI 增加 `moyuan repair deployment-risk review ...`、`queue` 和 `reviews`。
- API 增加 deployment risk review queue、review submit、review list/detail。
- 已补后端单测覆盖 pending queue、deferred、approved、重复 review 拒绝和 evidence 关联。

## 6. 执行规划：`phase17-004 console-policy-risk-drilldown`

实现状态：completed。

范围：

- Console 展示 release admission policy decision、matched rules 和 policy pack 摘要。
- Console 展示 rehearsal scheduler runs，包括 target、created/skipped/blocked/manual 计数和原因。
- Console 展示 deployment risk review queue、latest review 和 handoff drill-down。
- Console 只消费后端事实源，不在前端重新计算 policy、scheduler 或 review 状态。

验收：

- 用户能从 Deployment panel 进入 policy、scheduler 和 risk review 三个 drill-down。
- blocked/manual/skip 的原因可见。
- 前端 typecheck/build 通过，demo data 和 API normalize 同步更新。

完成记录：

- Console live snapshot 增加 release admission policy pack、rehearsal scheduler runs、deployment risk review queue 和 deployment risk reviews。
- Deployment panel 展示 admission policy id、matched rules、policy decision reason。
- Deployment panel 展示 scheduler run 的 created/skipped/blocked/manual 计数和 target reason。
- Deployment panel 展示 handoff review decision、review queue item 和 latest review。
- 新增受控 Scheduler 按钮，只调用后端 bounded scheduler，不在前端计算调度结果。
- Demo data 同步 Phase 17 policy/scheduler/risk review 事实源。

## 7. 执行规划：`phase17-005 phase17-readiness`

实现状态：completed。

范围：

- 统一 Phase 17 issue graph、实施记录、README、路线图和 readiness。
- 回归验证 release admission policy pack、bounded scheduler、risk review queue 和 Console 展示。
- 明确剩余风险、生产真实写入边界和 Phase 18 建议入口。

验收：

- 全量门禁通过。
- Phase 17 所有 issue 状态一致。
- Readiness 文档给出下一阶段入口和不允许绕过的安全边界。

完成记录：

- 新增 [Phase 17 Release Readiness](./phase17-release-readiness.md)，统一完成范围、门禁结论、保留边界和 Phase 18 建议入口。
- Phase 17 issue graph、实施记录、README、docs 索引和路线图均已同步为收口状态。
- 明确生产真实写入仍默认关闭，policy/scheduler/review/Console 均不能绕过 approval、authz、quality 和 provider gate。
