...
const diagramVariant = process.env.DIAGRAM_VARIANT || "default";
...
function buildDiagramSpec() {
  if (diagramVariant === "docs-structure-16x9") {
...
function writeExplanation(outBase, promptPath) {
  if (diagramVariant === "docs-structure-16x9") {
    writeDocsStructureExplanation(outBase, promptPath);
    return;
  }
...
function writeDocsStructureExplanation(outBase, promptPath) {
  const promptDisplayPath = path.relative(root, promptPath);
  const explanation = `# Moyuan Code 文档驱动全生命周期流程图讲解
