...
| --- | --- | --- | --- | --- | --- | --- |
| `phase22-001` | `guarded-write-execution-plan` | planned | 新增 write execution plan preview/apply 契约 | Phase 21 readiness | `backend_owner` + `security_owner` | Apply 需要 packet、approval、write switch，且不执行外部写入 |
| `phase22-002` | `write-execution-api-cli` | planned | 增加 API/CLI create/list 入口 | `phase22-001` | `backend_owner` | 可审计、可测试、失败可解释 |
| `phase22-003` | `console-write-execution-plan` | planned | Console 展示 execution plan | `phase22-002` | `frontend_owner` | 只读展示后端事实源 |
| `phase22-004` | `phase22-readiness` | planned | 收口验证、文档回写和后续真实 adapter 入口 | `phase22-003` | `release_owner` + `qa_owner` | 全量门禁通过，真实写入边界清晰 |
