...
| `phase11-004` | `quality-review-merge-queue` | completed | batch 质量聚合、review queue、merge decision queue、失败返工 | `phase11-002` | `qa_owner` | issue 只有 quality + review 通过后才能进入 merge ready |
| `phase11-005` | `console-batch-execution-surface` | completed | batch plan、batch runs、waiting reason、merge readiness Console 展示 | `phase11-001`,`phase11-004` | `frontend_owner` | Console 可查看 batch plan/run，并触发 dry-run preview |
...
当前执行图已完成。后续新增真实并发 worker、自动 merge、PR/MR 创建或 release batching 时，应新建后续 Phase，不再扩大 Phase 11 的范围。
...
- 没有 quality report、review decision 和 merge queue item ready，不允许进入 merge ready。
