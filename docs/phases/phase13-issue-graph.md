...
| `phase13-003` | `release-candidate-provider-preview` | completed | GitHub/Gitee provider preview、PR/MR plan、tag/release/workflow guarded actions | `phase13-001`,`phase13-002` | `git_owner` + `release_owner` | release candidate 可生成远程发布预览和 PR/MR handoff，不默认 push/tag/publish |
| `phase13-004` | `deployment-handoff-from-release-candidate` | completed | 根据 release candidate 生成 deployment plan、环境/服务器资源引用、smoke/monitor 模板引用 | `phase13-001` | `devops_owner` | release candidate 可进入部署 dry-run 和后续线上检查链路 |
| `phase13-005` | `console-release-candidate-surface` | completed | Console 展示 candidate、branch apply、provider preview、deployment handoff 和阻断原因 | `phase13-001`,`phase13-004` | `frontend_owner` | Console 可见 release candidate 全链路并只调用后端受控 API |
