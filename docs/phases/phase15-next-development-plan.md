...
| --- | --- | --- | --- | --- | --- |
| P0 | `phase15-001` | `deployment-execution-approval-proof` | planned | 部署真实执行审批凭证化 | 真实执行必须携带并消费匹配 approval |
| P0 | `phase15-002` | `rollback-execution-controller` | planned | 回退执行控制器 | rollback 默认 preview，真实执行 gated |
...
完成 `phase15-001` 后，应进入 `phase15-002 rollback-execution-controller`，把已有 rollback suggestion/runbook 接入独立的 preview、approval、write switch 和 evidence。
