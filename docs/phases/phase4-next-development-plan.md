...
| --- | --- | --- | --- | --- | --- |
| P0 | `phase4-001` | `audit-log-query-api-console` | planned | 统一核心日志查询 API 和 Console Audit 面板 | 脱敏后的 run/audit/error 日志可按 channel、issue、run、limit 查询 |
...
首个实现任务建议选择 `phase4-001 audit-log-query-api-console`。

原因：

- 它是 approval、team auth、Git PR/MR 和 server maintenance 的共同基础。
- 当前系统已经有 `.moyuan/logs/` 和 JSONL 事件，改动范围可控。
- Console 已有工作台骨架，可以小步增加 Audit 面板，便于后续高风险动作都能被用户看见。
