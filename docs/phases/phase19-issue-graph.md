...
| `phase19-003` | `durable-control-runner` | next | 为 bounded scheduler、resource scan、verification 和 audit export 增加幂等 run、retry budget 和失败恢复状态 | `phase19-002` | `orchestrator_owner` + `devops_owner` | 重试可控，重复触发不产生重复写入 |
