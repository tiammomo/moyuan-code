...
Subagent Manager 是 Orchestrator 内部的执行委派组件，负责把 ready issue、repair attempt、release task 或 memory job 转成受控 Subagent 实例。
...
- 分派 Runtime。
- 校验 Subagent 输出契约。
- 汇总 changed files、风险、测试和 memory candidates。
- 记录 Skill Effectiveness。

完整设计见 [Subagent 与 Skills 系统方案](./subagents-skills-system.md)。

### Agent Runtime

Agent Runtime 负责执行一个具体 Subagent 的任务。
