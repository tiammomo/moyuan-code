...
| --- | --- | --- | --- | --- | --- |
| P0 | `phase19-001` | `operations-audit-export` | next | 运维审计报告导出 | JSON/Markdown 可复盘且无 secret 泄漏 |
| P0 | `phase19-002` | `decision-ledger` | planned | 统一决策账本 | policy/readiness/verification/review 结论结构一致 |
| P1 | `phase19-003` | `durable-control-runner` | planned | 长期控制任务 runner | 幂等、重试、失败恢复可审计 |
