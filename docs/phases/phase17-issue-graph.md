...
| `phase17-004` | `console-policy-risk-drilldown` | completed | Console 展示 policy rules、scheduler run、risk handoff drill-down | `phase17-001`,`phase17-003` | `frontend_owner` | 前端只展示后端事实源和受控动作 |
| `phase17-005` | `phase17-readiness` | completed | 收口验证、文档回写、剩余风险和下一阶段入口 | `phase17-004` | `release_owner` + `security_owner` | 全量门禁通过，自动化边界清晰 |
