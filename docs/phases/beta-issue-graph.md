...
| `beta-004` | `parallel-orchestration-engine` | in_progress | 根据依赖、写入范围、runtime slot 和风险自动决定并发度 | `beta-003` | `scheduler_owner` | ready queue 可并发调度，冲突 issue 自动等待 |
...
当前执行 `beta-004 parallel-orchestration-engine`。

实现边界：

- 只做只读 API，不执行开发、合入、推送或部署。
- API 以 project id 解析被管理项目根目录。
- 有 GORM Store 时优先使用 Store 查询项目；无 Store 时回退到 `controlplane` 文件注册表。
- 状态事实仍以 `.moyuan/` 文件为准，GORM 先承担查询入口索引。
